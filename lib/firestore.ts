...
export async function markLectureComplete(
  ...
  try {
    ...
    await setDoc(ref, { completedAt: new Date().toISOString(), date }, { merge: true });
  } catch (err) {
...
export async function markTestAttempted(
  ...
  try {
    ...
    await setDoc(ref, { date, attemptedAt: new Date().toISOString() }, { merge: true });
  } catch (err) {
...
export async function getLectureProgress(uid: string): Promise<LectureProgress> {
  try {
    ...
    snapshot.forEach(docSnap => {
      ...
      result[docSnap.id] = { completedAt: data.completedAt ?? '', date: data.date ?? '' };
    });
    return result;
  } catch (err) {
...
export async function getTestProgress(uid: string): Promise<TestProgress> {
  try {
    ...
    snapshot.forEach(docSnap => {
      const data = docSnap.data();
      result[docSnap.id] = { date: data.date ?? '' };
    });
    return result;
  } catch (err) {
